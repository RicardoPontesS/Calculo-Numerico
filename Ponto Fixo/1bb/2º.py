...
def retornoFuncao(x):
    resultado = sp.exp(x - 2) + x**3 - x
    return resultado

def retornoFuncao2(x):
    resultado  = (x - sp.exp(x - 2))
    if(resultado<0):
...
def iteracoes(funcao_continua, intervalos):
    ...
    for intervalo in intervalos:
        ...
        while abs(xk - xk_anterior).evalf() > epsilon:
            convergencia = abs((xk - raiz) / (xk_anterior - raiz)).evalf()
            xk_anterior = xk
            xk = retornoFuncao2(xk_anterior)
            print(convergencia)
        print("------------------------------------------------------------------------------------")
...
def verificaIntervalo():
    ...
    while i < 5:  # Defina o limite superior adequado para a busca de intervalos
